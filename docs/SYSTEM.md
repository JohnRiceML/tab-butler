...
- **discovery + routing** — `findSpots("niche"|"fresh-reach")` (recent niche search; Fresh Reach pairs a focused Top lens with one guaranteed-distinct rotating exploration lens, merges decaying public distribution evidence into a persistent 30-day radar, supports a private max-12 massive watchlist, derives a max-10 massive-account keep-list from settled reply views, runs up to 12 due per-handle checks with four bounded watchlist lanes plus measured-winner/evidence-backed exploration coverage, keeps up to 24/two-per-author candidates through two content-scoring batches, then selects the best strict live/content opening per author), `urlPoll` (SPA navigation).
...
| `fresh-reach.ts` | Pure **Fresh Reach radar + massive keep-list** policy. `freshReachShortlist` admits only timely, settled, API-attributed X-reported views on actual Fresh Reach replies, applies a measured floor + two-outcome shrinkage, and caps at 10. Twelve due checks bound watchlist, measured-winner, and evidence-backed exploration coverage; fame-only accounts and recent repeat authors cannot force a slot. Public distribution evidence decays on a 14-day half-life. Posts become `early-fit` (<30 replies), `breakout` (strong distribution, ≤90m, <50 replies), or `major-early` (massive, ≤20m, <80 replies; live distribution required above 29), then require the strict 0.68 anchor/brief/`risk:none` content gate and final one-result-per-author selection. | `test-fresh-reach` |
