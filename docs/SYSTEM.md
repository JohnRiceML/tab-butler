...
for t in twttr policy hygiene pacing community momentum learn-stats supporters targets suggest-targets prompts activity draft-context profile-check; do node scripts/test-$t.mjs; done   # pure-lib unit tests
...
| `profile-check.ts` | The profile coach — measured findings on the CONVERSION surface (pinned-post rank vs your own best by X-reported views; bio presence), fed by a $0 own-profile DOM harvest. Silent without data. | `test-profile-check` |
