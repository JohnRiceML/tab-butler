...
4. Before drafting, each reply spot checks the last recorded assisted reply to that author. A reply within 24 hours substantially lowers all three recommendation lanes; one within 72 hours lowers them less. The feed pill, queue card, expanded score, and draft panel all call this out visibly.
...
- **Manual submit; legacy helper default off:** the default user-clicked handoff activates X's composer and prefills a draft for review, but the user submits it and then confirms it in Goobi. Legacy Like + insert is explicit opt-in/default-off and cannot run for Fresh reach; it also never submits the reply. These are private-use DOM helpers, not an official X API integration; public-release policy work is tracked in `docs/RELEASE-CHECKLIST.md`.
