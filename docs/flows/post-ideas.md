...
- Score breakout with `scoreWinner`: follower-normalized engagement (`eng / max(8, followers*0.003)`, a ~0.3% like-rate baseline) times `log10(eng+10)` so absolute pull still matters; unknown-follower posts compete against the median of the unknown subset. A small account's genuine breakout beats a mega-account's floor post.
...
- **`idea-quality.ts` is the unit-tested core** (Layer A, in the default gate). The thresholds (`TOO_SIMILAR`, `INPUT_DEDUP`, `COPY_LEAK`, the 0.3% like-rate baseline, the 21d/60d windows, the 40th-pct floor, ≤2/author, ≤40%/shape) are load-bearing tuning — changing them shifts which exemplars the model ever sees ("garbage in"). Keep `idea-quality.ts` import-free/DOM-free so the esbuild data-URL eval keeps working.
