...
## How it works
**Load:** `getData()` reads all `X_*` keys from `chrome.storage.local` (including the reply log and separate local pace-reset baseline) plus, if a data key is stored, a `GET_TWTTR_METER` round-trip. Reply stats come from the per-day `X_REPLY_LOG_KEY` buckets. Account safety rebuilds lane-tagged events and calls shared `replyPaceStatus(events, now, resetAt)`—a pure local read with no API.
