...
5. If the budget is nearly spent, instead of results they see a toast like "Monthly X-data budget nearly used — Find spots is paused. It resets on the 1st." Ambient reach lookups just quietly stop; cheap ones keep working.
6. They can check usage anytime in the popup: "This month: 142 MB / 10 GB · 1,240 / 100k requests."
...
- **Daily learn / measure pass:** `runMeasurePass()` (`x-copilot.ts:2176`) resolves rest_id via `path:"user"` then pulls `path:"user-replies-v2"` (intent), matches replies back to the sent log, and writes settled outcomes. dayKey-gated, idempotent; `refreshOwnStats` runs first so the two paths don't double-bill the `from:<handle>` fetch.
...
**State persisted:** `chrome.storage.local` — `twttrMeter` (monthly byte/request meter), `twttrKey` (BYO key), `X_MY_POSTS_KEY` (own-posts cache), `X_LEARN_STATS_KEY`, `X_REPLY_LOG_KEY`. The token bucket and `inflight`/`authorReach` maps are in-memory (reset on SW restart / navigation, which is acceptable for a burst guard).
...
- **Deliberately does NOT:** write anything (read-only enrichment — no posting, liking, following via this API); cache responses to storage (v1 is budget+rate only; callers dedupe in memory, a per-resource cache is planned v2); exceed budget — a `budget-*` block spends zero network and degrades gracefully rather than erroring loudly.
...
- **No response cache yet (v1).** Callers dedupe in memory via the cap/queue logic and the `inflight` coalescing; the per-resource TTLs in `TWTTR_CLASS` are declared but not yet enforced by a storage-backed cache (planned v2). Don't assume TTL-based caching is happening.
