...
5. **Token bucket:** `takeToken()` is an in-memory local smoothing ceiling at `RATE_PER_SEC = 8`, starting at 1 so a cold-start flood cannot burst. Provider response headers can impose a tighter window.
...
**Budget policy constants (`TWTTR_BUDGET`):** `BYTES = 8 GB` and `REQUESTS = 80_000` are conservative local UTC-month fallbacks, not claims about the user's RapidAPI plan. Provider quota headers join the gate whenever available. Thresholds remain CONSERVE 0.70 / FROZEN 0.85 / LOCKDOWN 0.95, with explicit user intent surviving longer than ambient work. RapidAPI's dashboard/billing cycle is the source of truth; the platform includes 10 GB per billing cycle before bandwidth fees rather than presenting 10 GB as a hard cap.
...
- **Find spots / ideas / Fresh Reach (intent):** explicit actions use `search-v3` with `intent:true`. Fresh Reach uses niche Latest, two Top account-discovery lenses, and up to 12 cached/direct account checks; exact query flights coalesce without allowing an unrelated auto query to swallow a manual scan. A query-specific Latest failure may use the saved radar, while credential/subscription/rate/provider failures stop. Budget errors point to the provider billing-cycle dashboard.
