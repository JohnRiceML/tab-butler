...
How each **flow** of the Goobi social copilots actually works — the user-facing path and the under-the-hood pipeline, grounded in the real code. Start here when you (or an agent) need to understand or change a feature.
...
### Core surfaces — Replies, Comments, and More tools
- [First-run X onboarding](x-onboarding.md) — resumable provider, consent, and focus setup, followed by a first-reply walkthrough.
- [LinkedIn comment opportunities](linkedin-comments.md) — focused LinkedIn v1: scan the feed, score comment fit, draft, and let Copy & review immediately record Commented/remove the opportunity without touching the composer; exact Undo remains available.
- [Reply spots](reply-spots.md) — the core copilot: scan the feed → Jev or Claude scores reply-worthiness → badge the good ones → Claude drafts a reply in your voice → copy it and open the exact post for a manual paste/review/post.
...
### Cross-cutting
- [Jev comment review](jev-comment-review.md) — optional, separately consented observation of X and LinkedIn drafts; measured fictional benchmark, limitations, and local review summaries.
- [Choosing posts and contributions](comment-selection.md) — grounded selection across X and LinkedIn, ready versus detail-required comments, and quality evaluation.
- [Commenting reliability](commenting-reliability.md) — priorities across both platforms, shared Claude request controls, cancellation, and recovery that preserves edits.
...
- [Target accounts + suggestions](targets.md) — optional cold discovery under More tools → Target accounts, after warm comments and high-fit reply spots are handled.
...
- [Daily goals + SOUL.md](daily-goals-and-soul.md) — the expandable Replies / Posts / DM-people scorecard and the user-owned beliefs/themes/boundaries creative brief.
