...
// ---- pickCallout: safety first, then measured trend, then the algo-backed nudges ----
{
  ...
  ok(m.pickCallout({ ...base, freeTier: true }).text.includes("free tier"), "a SET free tier → the labeled external-study callout (below every lever in priority)");
  ok(m.pickCallout({ ...base, trend: "cooling", freeTier: true }).text.includes("Cooling"), "measured levers still outrank the tier note");
  ok(!m.pickCallout(base).text.includes("free tier"), "unset tier → never mentioned (unknown ≠ free)");
