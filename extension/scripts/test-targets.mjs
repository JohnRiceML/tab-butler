...
// ---- thread-room heuristic (not an X probability) ----
ok(m.slotOdds(0) === 1 && m.slotOdds(m.EARLY_MAX_REPLIES) === 1, "an empty/near-empty thread has the most room");
ok(m.slotOdds(15) === 0.7, "a contested thread (6..29 replies) has less room");
ok(m.slotOdds(50) === 0.45 && m.slotOdds(500) === 0.3, "a crowded thread has progressively less room");
ok(m.slotOdds(undefined) === 0.65, "unknown reply count is usable but cannot look ideal");
