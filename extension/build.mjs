...
const iifeOpts = {
  entryPoints: {
    "x-copilot": "src/content/x-copilot.ts",
    "linkedin-copilot": "src/content/linkedin-copilot.ts",
  },
  bundle: true,
  format: "iife",
  target: "es2022",
...
function copyStatic(includeLocalSeed = false) {
  ...
  if (includeLocalSeed && existsSync("goobi.local.json")) cpSync("goobi.local.json", "dist/goobi.local.json");
  else rmSync("dist/goobi.local.json", { force: true });
}
...
if (!watch) {
  ...
  // open X/LinkedIn tabs on the way back up — no manual extension reload or tab refresh during dev.
  let devId = 0;
  let beaconTimer;
  const bumpBeacon = () => {
  ...
  await ctxA.watch();
  await ctxB.watch();
  console.log("[dev] watching src/ — load dist/ unpacked once; rebuilds hot-reload the extension and refresh open X/LinkedIn tabs. Ctrl-C to stop, then run `npm run build` to strip the dev beacon.");
}
